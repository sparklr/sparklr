...
function addNotification(notification) {
	...
	switch (notification.type) {
		case "1": //commented on post 
			notification.body = "<img class='avatar' src='" + getAvatar(notification.from) + "'>" + getDisplayName(notification.from) + " commented:<br>" + notification.body + "<br><br>";
			action = function() {
				location.href= "#/post/" + notification.from + "/" + notification.action + "/new";
			}
		break;
		case "2": //mentioned
			notification.body = "<img class='avatar' src='" + getAvatar(notification.from) + "'>" + getDisplayName(notification.from) + " mentioned you.<br><br>";
			action = function() { location.href = "#/post/" + notification.from + "/" + notification.action; }
		break;
		case "4": //wrote on board 
			notification.body = "<img class='avatar' src='" + getAvatar(notification.from) + "'>" + getDisplayName(notification.from) + " posted on your board:<br>" + notification.body + "<br><br>";
			action = function() { location.href = "#/user/" + curUser; }
		break;
...
function handleNotifications() {
	...
	for (id in currentNotifications) {
		...
		if (s[1] == "post" && s[3] == currentNotifications[id].action && (currentNotifications[id].type == N_EVENT || currentNotifications[id].type == N_MENTION)) {
...
function dismissNotification(id) {
	ajaxGet("work/dismissnotification.php?id=" + id);
